...
-   **EventPayloadDeserializer** - Fixed class import errors that broke GitHub Actions:
    -   Removed problematic imports of non-existent plural entity class names (`Users`, `Customers`, etc.)
    -   Simplified deserializer to return raw JSON:API formatted data directly
    -   Webhook payloads are already properly formatted from LemonSqueezy API, no deserialization needed
    -   Maintains backward compatibility with `registerType()` method
    -   Resolves PHP 8.1, 8.2, and 8.3 test failures on GitHub Actions
